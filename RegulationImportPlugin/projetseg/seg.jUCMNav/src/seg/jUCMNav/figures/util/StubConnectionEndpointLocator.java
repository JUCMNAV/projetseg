/**
 * Slight modifications of ConnectionEndPointLocator; if only they had used protected methods instead of private.
 * @author jkealey
 * 
 */
...
/**
 * Used to place IFigures along the endpoint or starting point of a {@link Connection}.<code>uDistance</code> represents the distance from the Connection's
 * owner to the IFigure. <code>vDistance</code> represents the distance from the IFigure to the Connection itself.
 */
public class StubConnectionEndpointLocator implements Locator {
    ...
    /**
     * Constructs a ConnectionEndpointLocator using the given {@link Connection}. If <i>isEnd </i> is <code>true</code>, the location is relative to the
     * Connection's end (or target) point. If <i>isEnd </i> is <code>false</code>, the location is relative to the Connection's start (or source) point.
     * 
     * @param c
     *            The Connection
     * @param isEnd
     ...
     * @since 2.0
     */
    public StubConnectionEndpointLocator(Connection c, boolean isEnd) {
        end = isEnd;
        conn = c;
        uDistance = 14;
        vDistance = 4;
    ...
    /*
     * Returns an integer representing the side of the passed Rectangle that a point lies on. 1 == Top 2 == Right 3 == Bottom 4 == Left
     * 
     ...
     * 
     * @param startPoint The starting point of the connection. @param endPoint The end point of the connection.
     */
    ...
    /*
     * Calculates 'tan' which is used as a factor for y adjustment when placing the connection label. 'tan' is capped at 1.0 in the positive direction and -1.0
    ...
    public void relocate(IFigure figure) {
        ...
        /*
         * Label placement calculations are done as if the connection point is along the left or right side of the figure. If the connection point is along the
         * top or bottom, values are transposed.
         */
     ...
     * 
     * @param distance
     *            Number of pixels to place the ConnectionEndpointLocator from its owner.
     * @since 2.0
     */
     ...
     * 
     * @param distance
     *            Number of pixels to place the ConnectionEndpointLocator from its Connection.
     * @since 2.0
     */
