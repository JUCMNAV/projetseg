 ...
 * <ul>
 *   <li>{@link jucm_OneFileWorkaround.ucm.map.impl.InBindingImpl#getBinding <em>Binding</em>}</li>
 *   <li>{@link jucm_OneFileWorkaround.ucm.map.impl.InBindingImpl#getStartPoint <em>Start Point</em>}</li>
 *   <li>{@link jucm_OneFileWorkaround.ucm.map.impl.InBindingImpl#getStubEntry <em>Stub Entry</em>}</li>
 *   <li>{@link jucm_OneFileWorkaround.ucm.map.impl.InBindingImpl#getPointcutExit <em>Pointcut Exit</em>}</li>
 * </ul>
 * </p>
 *
 * @generated
...
public class InBindingImpl extends EObjectImpl implements InBinding {
	private IwInBinding iwInBinding;
	
	@Override
	public void build() {
		iwInBinding = IntermediateWorkflowFactory.eINSTANCE.createIwInBinding();
	}
	...
	@Override
	public IwInBinding getIwInBinding() {
		return iwInBinding;
	}

	@Override
	public void linkPluginBinding() {
		getBinding().addIwInBinding(iwInBinding);
	}

	@Override
	public void linkDisjunctiveStubEntries() {
		LinkedHashSet<IwNodeConnection> nodeConnections = getStubEntry().iwDisjunctivePredNodeConnections();
		
		for(IwNodeConnection iwDisjunctivePredNodeConnection : nodeConnections){
			iwInBinding.getDisjunctiveStubEntries().add(iwDisjunctivePredNodeConnection);
		}
	}

	@Override
	public void linkStartPoint() {
		if(getPointcutExit() == null) 
			iwInBinding.setPluginStartPoint(getStartPoint().getIwStartPoint());
		else {
			IwStartPoint iwSp = iwGetPluginStartPointFromPointcutStub();
			/*if(iwSp!=null)*/
				iwInBinding.setPluginStartPoint(iwSp);	
		}
	}

	@Override
	public IwStartPoint iwGetPluginStartPointFromPointcutStub() {
		PathNode pointcutStub = getPointcutExit().sourceAsPathNode();
		/*Stub stb = (Stub)pointcutStub;
		if(stb.isReplacementPointcutStub())
			return null;
		else*/
			return (IwStartPoint)(pointcutStub.iwGetExitNode(getPointcutExit()));
	}
