...
import jucm_OneFileWorkaround.ucm.map.FailureKind;
 ...
 * <ul>
 *   <li>{@link jucm_OneFileWorkaround.ucm.map.impl.StartPointImpl#getWorkload <em>Workload</em>}</li>
 *   <li>{@link jucm_OneFileWorkaround.ucm.map.impl.StartPointImpl#isLocal <em>Local</em>}</li>
 *   <li>{@link jucm_OneFileWorkaround.ucm.map.impl.StartPointImpl#getFailureKind <em>Failure Kind</em>}</li>
 *   <li>{@link jucm_OneFileWorkaround.ucm.map.impl.StartPointImpl#getInBindings <em>In Bindings</em>}</li>
 *   <li>{@link jucm_OneFileWorkaround.ucm.map.impl.StartPointImpl#getPrecondition <em>Precondition</em>}</li>
 *   <li>{@link jucm_OneFileWorkaround.ucm.map.impl.StartPointImpl#getScenarioStartPoints <em>Scenario Start Points</em>}</li>
 * </ul>
 * </p>
 *
 * @generated
...
public class StartPointImpl extends PathNodeImpl implements StartPoint {
	private IwStartPoint iwStartPoint;
	
	public void link() {
		if(hasReplacementPointcutStubAsTarget()==false) {
			super.link();
			getUCMmap().getIwWorkflow().getStartPoints().add(iwStartPoint);
		}
	}
	
	@Override
	public void buildIwNodeTemplate() {
		iwStartPoint = IntermediateWorkflowFactory.eINSTANCE.createIwStartPoint();
		iwStartPoint.setName(nameOrPrefixId("StartPoint"));
		iwStartPoint.setStartPointType(iwStartPointType());
		addIwEquivalentNodeBeforeOutIn(iwStartPoint);	
	}

	@Override
	public IwStartPoint getIwStartPoint() {
		return iwStartPoint;
	}

	@Override
	public String iwStartPointType() {
		if(getInBindings().size() > 0) 
			return "bound";
		else {
			if(isLocal())
				return "local";
	...
	@Override
	public boolean hasReplacementPointcutStubAsTarget() {
		Iterator<NodeConnection> itr = succAsNodeConnection().iterator();
		PathNode target = (PathNode)itr.next().getTarget();
		
		//if(target != null) {
		if(target instanceof Stub) {
			Stub stub = (Stub)target;
			return stub.isReplacementPointcutStub();
		}
		//}
		return false;
	}
	 ...
	 * @ordered
	 */
	protected static final FailureKind FAILURE_KIND_EDEFAULT = FailureKind.NONE;
	 ...
	 * @ordered
	 */
	protected EList<ScenarioStartPoint> scenarioStartPoints;
	...
	public EList<InBinding> getInBindings() {
		if (inBindings == null) {
			inBindings = new EObjectWithInverseResolvingEList<InBinding>(InBinding.class, this, MapPackage.START_POINT__IN_BINDINGS, MapPackage.IN_BINDING__START_POINT);
		}
		return inBindings;
	}
	 ...
	 * @generated
	 */
	public EList<ScenarioStartPoint> getScenarioStartPoints() {
		if (scenarioStartPoints == null) {
			scenarioStartPoints = new EObjectWithInverseResolvingEList<ScenarioStartPoint>(ScenarioStartPoint.class, this, MapPackage.START_POINT__SCENARIO_START_POINTS, ScenarioPackage.SCENARIO_START_POINT__START_POINT);
		}
	...
	public NotificationChain eInverseAdd(InternalEObject otherEnd, int featureID, NotificationChain msgs) {
		switch (featureID) {
			...
			case MapPackage.START_POINT__SCENARIO_START_POINTS:
				return ((InternalEList<InternalEObject>)(InternalEList<?>)getScenarioStartPoints()).basicAdd(otherEnd, msgs);
		}
	...
	public void eSet(int featureID, Object newValue) {
		switch (featureID) {
			...
			case MapPackage.START_POINT__SCENARIO_START_POINTS:
				...
				getScenarioStartPoints().addAll((Collection<? extends ScenarioStartPoint>)newValue);
				return;
		}
