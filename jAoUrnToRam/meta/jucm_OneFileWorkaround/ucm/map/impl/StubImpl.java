 ...
 * <ul>
 *   <li>{@link jucm_OneFileWorkaround.ucm.map.impl.StubImpl#isDynamic <em>Dynamic</em>}</li>
 *   <li>{@link jucm_OneFileWorkaround.ucm.map.impl.StubImpl#isShared <em>Shared</em>}</li>
 *   <li>{@link jucm_OneFileWorkaround.ucm.map.impl.StubImpl#getRepetitionCount <em>Repetition Count</em>}</li>
 *   <li>{@link jucm_OneFileWorkaround.ucm.map.impl.StubImpl#isPointcut <em>Pointcut</em>}</li>
 *   <li>{@link jucm_OneFileWorkaround.ucm.map.impl.StubImpl#isSynchronization <em>Synchronization</em>}</li>
 *   <li>{@link jucm_OneFileWorkaround.ucm.map.impl.StubImpl#isBlocking <em>Blocking</em>}</li>
 *   <li>{@link jucm_OneFileWorkaround.ucm.map.impl.StubImpl#getAopointcut <em>Aopointcut</em>}</li>
 *   <li>{@link jucm_OneFileWorkaround.ucm.map.impl.StubImpl#getAspect <em>Aspect</em>}</li>
 *   <li>{@link jucm_OneFileWorkaround.ucm.map.impl.StubImpl#getBindings <em>Bindings</em>}</li>
 * </ul>
 * </p>
 *
 * @generated
...
public class StubImpl extends PathNodeImpl implements Stub {
	...
	public void build(){
		...
		getStrategy().build();
		//if(isReplacementPointcutStub() == false) {
	...
	public boolean iwHasNodes() {
		return getStrategy().hasNodes();
	}
	
	@Override
	public IwNode iwGetEntryNode(NodeConnection nodeConnection) {
		return getStrategy().setEntryNode(nodeConnection);
	}
	
	@Override
	public IwNode iwGetExitNode(NodeConnection nodeConnection) {
		return getStrategy().setExitNode(nodeConnection);
	}
	
	@Override
	public void addIwPluginBinding(IwPluginBinding iwPluginBinding) {
		//PreCondition: isPointcutStub==false
		if(isPointcutStub()==false){
			IwStub iwStub = (IwStub)iwGetEntryNode(null);
			iwStub.getPluginBindings().add(iwPluginBinding);
		}
	}
	...
	@Override
	public boolean isRegularPointcutStub() {
		return getAopointcut().equals(PointcutKind.REGULAR);
	}

	@Override
	public boolean isReplacementPointcutStub() {
		return getAopointcut().equals(PointcutKind.REPLACEMENT);
	}

	@Override
	public String stubNameOrPrefixId() {
		return nameOrPrefixId("Stub");
	}
	 ...
	 * @ordered
	 */
	protected static final PointcutKind AOPOINTCUT_EDEFAULT = PointcutKind.NONE;
	 ...
	 * @ordered
	 */
	protected static final AspectKind ASPECT_EDEFAULT = AspectKind.NONE;
	...
	public EList<PluginBinding> getBindings() {
		if (bindings == null) {
			bindings = new EObjectContainmentWithInverseEList<PluginBinding>(PluginBinding.class, this, MapPackage.STUB__BINDINGS, MapPackage.PLUGIN_BINDING__STUB);
		}
		return bindings;
	}
	...
	public void eSet(int featureID, Object newValue) {
		switch (featureID) {
			...
			case MapPackage.STUB__REPETITION_COUNT:
				...
				return;
			case MapPackage.STUB__POINTCUT:
				setPointcut((Boolean)newValue);
				return;
			case MapPackage.STUB__SYNCHRONIZATION:
				setSynchronization((Boolean)newValue);
				return;
			case MapPackage.STUB__BLOCKING:
