 ...
 * 
 */
public class ForColorUtils {
  static ForSliceColorList currentList=null;
	public static boolean sliceON=false;
	static UCMNavMultiPageEditor editor;
	public ForColorUtils() {
	...
	public static void doColor()
	{
		...
		IWorkbenchPage page = PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage();
		   ...
		   for(ForSliceColorList element:ForSlicingAlg.colorList)
		   {
			...
			//compare the diagrams to get the right editor
			  if(pageEditor.getModel().equals(((PathNode) element).getDiagram()))
			  {
				   ...
				   if(modelEditPart !=null)
				   {
					   ...
					   EList connections=element.getSucc();
				    	if(connections!=null)
	...
	public static void unColor(ForSliceColorList element)
	{
	...
	{
		EditPart modelEditPart;
	   if(!ForSlicingAlg.criterionExcludedNC.isEmpty())
		{
		   NodeConnection nc=ForSlicingAlg.criterionExcludedNC.get(0);
		for(int i=0; i<editor.getPageCount();i++)
		{
			//how to get the editpart of the model element
			UrnEditor pageEditor=(UrnEditor) editor.getEditor(i);
			//compare the diagrams to get the right editor
			  ...
			  {
				  for(NodeConnection nodeconnection:ForSlicingAlg.criterionExcludedNC)
				  {
